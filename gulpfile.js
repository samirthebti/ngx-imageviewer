...
const ngc = (args) => new Promise((resolve)=>{// Promisify version of the ngc compiler
...
gulp.task('clean:dist', () => {
  return del(config.outputDir).catch(() => {});
});

gulp.task('clean:build', () => {
  return del(config.buildDir).catch(() => {});
});

gulp.task('clean:coverage', () => {
  return del(config.coverageDir).catch(() => {});
});

gulp.task('clean:doc', () => {
  return del(`${config.outputDir}/doc`).catch(() => {});
});
...
gulp.task('rollup-bundle', () => {
  ...
  .then(() => {
    ...
    const globals = {
      ...
      'pdfjs-dist': _.camelCase('pdfjs-dist'.replace('/','.')),
      'hammerjs': _.camelCase('hammerjs'.replace('/','.'))
