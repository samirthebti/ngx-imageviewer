import { Component, Input, ViewChild, AfterViewInit, Renderer, Inject, OnDestroy, ElementRef } from '@angular/core';
...
export class ImageViewerComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this._nextPageButton.onClick = () => { this.nextPage(); return false; };
    this._beforePageButton.onClick = () => { this.previousPage(); return false; };
    this._zoomOutButton.onClick = () => { this.zoomOut(); return false; };
    this._zoomInButton.onClick = () => { this.zoomIn(); return false; };
    this._rotateLeftButton.onClick = () => { this.rotateLeft(); return false; };
    this._rotateRightButton.onClick = () => { this.rotateRight(); return false; };
    this._resetButton.onClick = () => { this.resetImage(); return false; };
  ...
  processTouchEvent(evt: HammerInput) {
    // process pan
  ...
  private addEventListeners() {
    // zooming
    this._listenDestroyList.push(this._renderer.listen(this._canvas, 'DOMMouseScroll', (evt: MouseWheelEvent) => this.onMouseWheel(evt)));
    this._listenDestroyList.push(this._renderer.listen(this._canvas, 'mousewheel', (evt: MouseWheelEvent) => this.onMouseWheel(evt)));

    // show tooltip when mouseover it
    this._listenDestroyList.push(this._renderer.listen(this._canvas, 'mousemove', (evt: MouseEvent) =>
      this.checkTooltipActivation(this.screenToCanvasCentre({ x: evt.clientX, y: evt.clientY }))
    ));
  }

  private onMouseWheel(evt: MouseWheelEvent) {
    evt.preventDefault();
  ...
  private drawPaginator(ctx: CanvasRenderingContext2D) {
    const padding = this.config.tooltips.padding;
    const radius = this.config.tooltips.radius;
    const labelWidth = 50;
