...
export class Button {
  ...
  // drawn on position
  private drawPosition: { x: number; y: number; } = null;
  ...
  //#region Events
  // click action
  onClick(evt?: HammerInput) { alert('no click action set!'); return true; }
  ...
  private drawIconFont(ctx: CanvasRenderingContext2D, centreX: number, centreY: number, size: number) {
    // font settings
...
export abstract class ResourceLoader {
  ...
  public draw(
    ...
    canvasDim: Dimension,
    onFinish: (ctx: CanvasRenderingContext2D, cfg: ImageViewerConfig, dim: Dimension) => void
  ) {
    // clear canvas
  ...
  public onResourceChange(): Observable<string> {
